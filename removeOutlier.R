...
for(freq in BandFreq){
    ...
    for(src in sourceList){
        if( substr(src, 1,1) != 'J' ){ next }   # Filter SSOs out
        ...
        for(index in 1:nrow(fluxDF)){
            ...
            if( abs(median(srcDF[srcDF$Date == fluxDF[index,]$Date,]$I) -  fluxDF[index,]$I) > 0.5* fluxDF[index,]$I){  
                cat(sprintf('%s : %s Iobs= %.1f Jy / Ipred= %.1f Jy\n', srcDF[srcDF$Date == fluxDF[index,]$Date,]$File, src, srcDF[srcDF$Date == fluxDF[index,]$Date,]$I, fluxDF[index,]$I ))
